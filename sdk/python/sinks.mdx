...
| `api_key` | `str` | Optional | API key for authentication (backward compatible) |
| `auth_type` | `str` | `"bearer"` | Authentication type: `bearer`, `basic`, `header`, or `none` |
| `auth_value` | `str` | Optional | Auth token or credentials |
...
The `HttpBatchSink` supports multiple authentication methods via the `auth_type` and `auth_value` parameters. The legacy `api_key` parameter continues to work for backward compatibility.

#### Bearer token (default)

Used for Arzule cloud and services expecting `Authorization: Bearer <token>` headers.
...
For services using HTTP Basic Auth. Provide credentials as `username:password`.

```python
sink = HttpBatchSink(
    endpoint_url="https://my-backend.com/ingest",
    auth_type="basic",
...
For services requiring custom authentication headers. Provide headers as a JSON string.

```python
sink = HttpBatchSink(
    endpoint_url="https://my-backend.com/ingest",
    auth_type="header",
    auth_value='{"X-Api-Key": "my-key"}'
)
```

#### No authentication

For local development or internal services that do not require authentication.
